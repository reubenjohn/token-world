...
class TestIlluminationMetadata:
    ...
    def test_is_voluntary_for_routing(self, mechanic: IlluminationMechanic) -> None:
        """Phase-4 routing: voluntary=True so UC-V06's verb can match.

        Semantic intent is reactive / involuntary (recorded by the
        ``involuntary_intent`` tag and retained ``watches()``); see
        fire_spread for the identical rationale.
        """
...
class TestIlluminationCheck:
    ...
    def test_check_fails_when_target_missing(self, mechanic: IlluminationMechanic) -> None:
        kg = KnowledgeGraph()
...
class TestIlluminationApply:
    ...
    def test_apply_sums_multiple_lit_sources(self, mechanic: IlluminationMechanic) -> None:
        kg = KnowledgeGraph()
        kg.add_node("room", node_type="entity", subtype="room", illumination=0)
        kg.add_node("torch_a", node_type="entity", subtype="torch", lit=True, light_radius=5)
        kg.add_node("torch_b", node_type="entity", subtype="torch", lit=True, light_radius=3)
        kg.add_node("torch_off", node_type="entity", subtype="torch", lit=False, light_radius=7)
    ...
    def test_apply_recomputes_to_zero_when_source_extinguished(
        ...
        kg.add_node("torch", node_type="entity", subtype="torch", lit=False, light_radius=5)
    ...
    def test_apply_is_idempotent_when_value_unchanged(self, mechanic: IlluminationMechanic) -> None:
        ...
        kg.add_node("room", node_type="entity", subtype="room", illumination=5)
        kg.add_node("torch", node_type="entity", subtype="torch", lit=True, light_radius=5)
        kg.add_edge("torch", "room", relation="located_in")
        ctx = MechanicContext(kg, actor="torch", target="torch")
        muts = mechanic.apply(ctx)
        assert muts == []
...
class TestIlluminationChain:
    def test_chain_light_torch_propagates_illumination(
        ...
        engine = ChainExecutionEngine(involuntary_mechanics=[IlluminationMechanic()], max_depth=10)
