...
from token_world.engine.config import EngineConfig, generate_universe_seed, load_engine_config
...
class TestLoadEngineConfigMalformed:
    ...
    def test_malformed_yaml_returns_defaults(self, tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
        ...
        assert cfg.max_chain_depth == 10
        assert cfg.universe_seed == 0
        assert any("Malformed" in r.message or "malformed" in r.message.lower() for r in caplog.records)

    def test_root_is_list_returns_defaults(self, tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    def test_universe_seed_not_int_uses_zero(self, tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
