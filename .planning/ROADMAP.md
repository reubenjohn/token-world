...
- [x] **Phase 04.1: Operator Agent Harness** (INSERTED) - Agent SDK driver that connects the simulation's yield→author→resume loop so an operator model can receive yielded action contexts, author a mechanic in Python, and resume ticks; end-to-end operator-side orchestration that Phase 5 depends on
