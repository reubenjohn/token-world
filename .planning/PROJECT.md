...
| Hybrid SDK: Agent SDK orchestrates, raw API inside tools | Agent SDK (Opus) sits at the top as operator/collaborator. Simulation runs as minimal MCP tools (resume_tick, rollback, list_mechanics) powered by raw API calls underneath. Operator implements mechanics natively via file writes + subagents. Raw API handles deterministic pipeline calls (classification, matching, observation). Human can collaborate via same tool interface. | -- Pending |
| Mechanics as flat Python modules (normal SDLC) | Each mechanic is `mechanics/<id>.py` with class attributes (id, description, voluntary, tags); shared helpers as `_*.py` modules; tests in project test tree. Registry imports modules and collects Mechanic subclasses. Git provides per-file history. Authoring feels like a normal codebase — refactoring, code reuse, and test-driven development work naturally. | -- Pending |
...
| Opus-via-Agent-SDK authors mechanics; Sonnet/Haiku power engine classification | The top-level coding agent (Opus via Agent SDK) is the mechanic author using normal SDLC — not a bespoke generation pipeline; Haiku/Sonnet handle inner-loop classification and observation. | -- Pending |
