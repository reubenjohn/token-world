...
| 04-11-T1 | 04-11 | 4 | MECH-03 | T-04-CYCLE | MECH20/22/23 fire_spread + decay_tick + illumination; reactive-cycle guards (do not reignite already-burning; idempotent illumination recompute); fire_spread + illumination voluntary=True for Phase-4 harness routing (documented deviation, involuntary_intent tag) | unit | `uv run pytest tests/test_mechanic/test_seeds/test_fire_spread.py tests/test_mechanic/test_seeds/test_decay_tick.py tests/test_mechanic/test_seeds/test_illumination.py -x -q` | ✓ | ✅ passing |
| 04-11-T2 | 04-11 | 4 | MECH-03 | T-04-STUB-IMPORT-LEAK | MECH24 contagion (real; transmission_rate deterministic edges; reactive-cycle guard; GAP-GRAPH05 seeded-RNG workaround) + MECH21 weather_reaction D-38 stub (blocked_by=GAP-ENG09); both validate clean through six-stage pipeline | unit | `uv run pytest tests/test_mechanic/test_seeds/test_contagion.py tests/test_mechanic/test_seeds/test_weather_reaction.py -x -q` | ✓ | ✅ passing |
| 04-11-T3 | 04-11 | 4 | MECH-03, TEST-02 | — | UC-V01/V06/V07 flip to pass (verb rewires to fire_spread / illumination / contagion; UC-V06 manifest reshaped single-tick per Phase-4 harness final-state assertion model); UC-V02/V04 route via stub to GAP-ENG09; UC-V03 stays blocked per PLAN decision tree with inline GAP-ENG07 rationale | integration | `uv run pytest tests/test_integration/test_use_cases.py -k "UC-V01 or UC-V02 or UC-V03 or UC-V04 or UC-V06 or UC-V07" -q` | ✓ | ✅ passing |
